...
export class AddRoleComponent implements OnInit {
  ...
  addRole(): void {
    ...
    this._httpService.postRB('api/v1/workflow/add-role', model).subscribe(res => {
      if (res['status'] === 200) {
        this.loading = false;
  ...
  editRoleDetails(): void {
    ...
    this._httpService.post('user-management/api/v1/workflow/edit/role', model).subscribe(res => {
      if (res['status'] === 200) {
