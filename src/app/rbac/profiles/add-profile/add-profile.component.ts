...
export class AddProfileComponent implements OnInit {
  ...
  addProfile(): void {
    ...
    this._httpService.post('user-management/api/v1/workflow/add-profile', model).subscribe(res => {
      if (res['status'] === 200) {
  ...
  editProfileDetails(): void {
    ...
    this._httpService.post('user-management/api/v1/workflow/edit/profile/', model).subscribe(res => {
      if (res['status'] === 200) {
